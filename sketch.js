...
bellAnimation=loadImage("bell.png");
upAnimation=loadImage("up.png")
...
function setup(){
 ...
 bell= createSprite(width-100,height-300,40,40);
 bell.addAnimation("bell",bellAnimation)
 bell.scale=0.1;

 up= createSprite(width-100,height-150,40,40);
 up.addAnimation("up",upAnimation)
 up.scale=0.5;
...
cicleman = createSprite(200,550,60,50);
...
function draw() {
  ...
  if (gameState === PLAY) {
    ...
    if (mousePressedOver(bell)|| keyDown("A")) {
      bellSound.play();
    }
   ...
   if (cicleman.isTouching(CashesGroup)) {
    ...
    if(mousePressedOver(up) && cicleman.y  >= height-120 || (touches.length > 0|| keyDown("SPACE")) && cicleman.y  >= height-120) {
      jumpSound.play ();
